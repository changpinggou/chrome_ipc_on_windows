...
DWORD GetParentProcessID(DWORD dwId)
{
	...
	NtQueryInformationProcess = (PNTQUERYINFORMATIONPROCESS)GetProcAddress(handle, "NtQueryInformationProcess");  
...
// static
bool IsLaunchFromeExplorerProc(){
	DWORD nID   = GetCurrentProcessId();
	DWORD nParentID = GetParentProcessID(nID);

	std::wstring strProcessPath;
	...
	if(::Process32FirstW(hSnapshot, &procinfo)){
		do{
			strProcessPath = procinfo.szExeFile; 
			if (strProcessPath == L"explorer.exe"){
				if(nParentID == procinfo.th32ProcessID){
					return true;
				}
			} 
...
bool ProcessMetrics::GetWorkingSetKBytes(WorkingSetKBytes* ws_usage) const {
  ...
  DWORD entries = static_cast<DWORD>(buffer->NumberOfEntries);
  number_of_entries = number_of_entries < entries ? number_of_entries : entries;
