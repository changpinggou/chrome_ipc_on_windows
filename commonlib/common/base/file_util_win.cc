...
int64 FileEnumerator::GetFilesize(const FindInfo& find_info) {
  ...
  DCHECK_LE(size.QuadPart, LONG_MAX);
...
int GetMaximumPathComponentLength(const FilePath& path) {
  ...
  int temp = MAX_PATH - 1 - static_cast<int>(prefix);
  int whole_path_limit = 0 > temp ? 0 :temp;
  return whole_path_limit < static_cast<int>(max_length) ? whole_path_limit : static_cast<int>(max_length);
}
